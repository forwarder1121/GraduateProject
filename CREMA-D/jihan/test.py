...
def main():
    ...
    parser.add_argument('--feature_type', type=str, required=True, choices=['mfcc'], help='Feature type used during training')
    ...
    # 경로 설정
    base_dir = f'/workspace/UndergraduateResearchAssistant/GraduateProject/code/CREMA-D/jihan/{args.feature_type}/{args.model_type}/lr_{args.learning_rate}'
    ...
    if args.feature_type == 'mfcc':
        ...
        test_features, test_labels, test_filenames = read_test_file_list(
            root='/workspace/dataset/CREMA-D/test_without_XX',
            n_mfcc=args.n_mfcc,
            n_mels=args.n_mels,
            feature_type='mfcc',
    ...
    for fold in range(args.n_folds):
        ...
        with torch.no_grad():
            for features, emotion, filename in test_loader:
                ...
                # 입력 데이터 차원 변경 (채널 차원 추가 및 순서 변경)
                if features.dim() == 2:
                    features = features.unsqueeze(0)
                features = features.permute(0, 2, 1)  # (batch_size, channels, seq_len)
