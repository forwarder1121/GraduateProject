...
emotions = ['ANG', 'DIS', 'FEA', 'SAD', 'HAP', 'NEU']

def read_train_file_list(root='/workspace/dataset/CREMA-D', n_mfcc=20):
    data_path = root
    file_paths = []
    ...
    for file_path in file_paths:
        ...
        # 파일 이름에 따라 감정을 분류
        if 'HAP' == file_path.split('_')[2] or 'NEU' == file_path.split('_')[2]:
            emotion = 0  # NotStressed
        elif 'ANG' == file_path.split('_')[2] or 'DIS' == file_path.split('_')[2] or 'SAD' == file_path.split('_')[2] or 'FEA' == file_path.split('_')[2]:
            emotion = 1  # Stressed
        else:
...
def get_kfold_data(k_folds=5, random_state=1, root='/workspace/dataset/CREMA-D', n_mfcc=20):
...
if __name__ == "__main__":
    try:
        k_folds_data = get_kfold_data(k_folds=5, random_state=1, root='/workspace/dataset/CREMA-D', n_mfcc=20)
